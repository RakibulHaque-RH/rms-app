...
<head>
    ...
    <style>
        :root {
            ...
            --dark: #0f172a; --content-bg: #f1f5f9; --card-bg: #ffffff;
            --text-primary: #1e293b; --text-secondary: #64748b; --text-muted: #94a3b8;
            --border: #e2e8f0; --radius: 12px; --radius-sm: 8px;
        }
        * { margin:0; padding:0; box-sizing:border-box; }
        body { font-family:'Inter',system-ui,sans-serif; background:var(--content-bg); color:var(--text-primary); overflow-x:hidden; }
        ...
        .brand-icon {
            ...
            font-size:20px; color:#fff; box-shadow:0 4px 12px rgba(99,102,241,.4); flex-shrink:0;
        }
        .brand-text { color:#fff; font-size:19px; font-weight:700; letter-spacing:-.5px; }
        .brand-sub { color:rgba(255,255,255,.45); font-size:11px; font-weight:500; text-transform:uppercase; letter-spacing:1px; }

        .sidebar-menu { padding:16px 12px; flex:1; }
        .menu-label {
            color:rgba(255,255,255,.3); font-size:11px; font-weight:600;
            text-transform:uppercase; letter-spacing:1.5px; padding:16px 16px 8px;
        }
        .menu-item {
            display:flex; align-items:center; gap:14px; padding:11px 16px;
            color:rgba(255,255,255,.6); text-decoration:none; border-radius:var(--radius-sm);
            margin-bottom:2px; transition:all .2s; font-size:14px; font-weight:500; position:relative;
        }
        .menu-item:hover { background:rgba(255,255,255,.08); color:#fff; transform:translateX(3px); }
        .menu-item.active {
            background:linear-gradient(135deg,rgba(99,102,241,.3),rgba(139,92,246,.2));
            color:#fff; box-shadow:0 0 20px rgba(99,102,241,.12);
        }
        .menu-item.active::before {
            content:''; position:absolute; left:0; top:50%; transform:translateY(-50%);
            width:3px; height:22px; background:var(--primary-light); border-radius:0 3px 3px 0;
        }
        .menu-item i { width:22px; text-align:center; font-size:15px; }
        .menu-item .badge { margin-left:auto; font-size:10px; padding:3px 8px; border-radius:20px; }
        ...
        .sidebar-footer .avatar {
            ...
            background:linear-gradient(135deg,var(--primary),var(--secondary)); color:#fff; font-size:14px; font-weight:600;
        }
        .sidebar-footer .user-name { color:#fff; font-size:13px; font-weight:600; }
        .sidebar-footer .user-role { color:rgba(255,255,255,.45); font-size:11px; text-transform:capitalize; }
        ...
        .nav-btn {
            ...
            color:var(--text-secondary); cursor:pointer; transition:all .2s; position:relative;
        }
        ...
        /* Cards */
        .card { background:var(--card-bg); border:1px solid var(--border); border-radius:var(--radius); box-shadow:0 1px 2px rgb(0 0 0/.05); }
        .card-header { padding:18px 24px; background:transparent; border-bottom:1px solid var(--border); font-weight:600; }
        ...
        .stat-card:hover { transform:translateY(-4px); box-shadow:0 10px 25px rgb(0 0 0/.08); }
        .stat-icon {
            width:52px; height:52px; border-radius:14px; display:flex; align-items:center;
            justify-content:center; font-size:22px; margin-bottom:16px;
        }
        .stat-card.primary .stat-icon { background:rgba(99,102,241,.1); color:var(--primary); }
        ...
        /* Table */
        .table thead th {
            background:#f8fafc; border-bottom:2px solid var(--border); color:var(--text-secondary);
            font-weight:600; font-size:12px; text-transform:uppercase; letter-spacing:.5px; padding:14px 16px;
        }
        .table tbody td { padding:14px 16px; vertical-align:middle; border-bottom:1px solid var(--border); font-size:14px; }
        ...
        .status-badge.preparing::before { background:#3b82f6; }
        .status-badge.ready { background:#e0e7ff; color:#3730a3; }
        .status-badge.ready::before { background:#6366f1; }
        .status-badge.served, .status-badge.completed { background:#d1fae5; color:#065f46; }
        .status-badge.served::before, .status-badge.completed::before { background:#10b981; }
        .status-badge.cancelled { background:#fee2e2; color:#991b1b; }
        .status-badge.cancelled::before { background:#ef4444; }
        .status-badge.available { background:#d1fae5; color:#065f46; }
        .status-badge.available::before { background:#10b981; }
        .status-badge.occupied { background:#fee2e2; color:#991b1b; }
        .status-badge.occupied::before { background:#ef4444; }
        .status-badge.reserved { background:#dbeafe; color:#1e40af; }
        .status-badge.reserved::before { background:#3b82f6; }
        .status-badge.maintenance { background:#f3f4f6; color:#374151; }
        .status-badge.maintenance::before { background:#6b7280; }
        ...
        .btn-primary {
            ...
            box-shadow:0 2px 8px rgba(99,102,241,.3); transition:all .3s;
        }
        .btn-primary:hover {
            background:linear-gradient(135deg,var(--primary-dark),#4338ca);
            transform:translateY(-1px); box-shadow:0 4px 16px rgba(99,102,241,.4);
        }
        ...
        /* Alert */
        .alert-dismissible { border-radius:var(--radius-sm); border:none; font-size:14px; font-weight:500; }

        /* Table cards (restaurant tables) */
        .table-card {
            border-radius:var(--radius); padding:20px; text-align:center; transition:all .3s;
            border:2px solid var(--border); cursor:pointer; position:relative; overflow:hidden;
        }
        .table-card:hover { transform:translateY(-4px); box-shadow:0 8px 24px rgb(0 0 0/.08); }
        .table-card.available { border-color:#86efac; background:linear-gradient(135deg,#f0fdf4,#dcfce7); }
        .table-card.occupied { border-color:#fca5a5; background:linear-gradient(135deg,#fef2f2,#fee2e2); }
        .table-card.reserved { border-color:#93c5fd; background:linear-gradient(135deg,#eff6ff,#dbeafe); }
        .table-card.maintenance { border-color:#d1d5db; background:linear-gradient(135deg,#f9fafb,#f3f4f6); }
        .table-card .table-num { font-size:24px; font-weight:800; margin:8px 0 4px; }
        .table-card .table-cap { font-size:12px; color:var(--text-secondary); }
...
<body>
    ...
    <aside class="sidebar" id="sidebar">
        ...
        <nav class="sidebar-menu">
            ...
            <a href="{{ route('staff.index') }}" class="menu-item {{ request()->routeIs('staff.*') ? 'active' : '' }}">
        ...
        <div class="sidebar-footer">
            ...
            <form action="{{ route('logout') }}" method="POST" class="ms-auto">
                @csrf
                <button type="submit" class="nav-btn" style="width:32px;height:32px;border:none;background:rgba(255,255,255,.1);color:rgba(255,255,255,.5);" title="Logout">
